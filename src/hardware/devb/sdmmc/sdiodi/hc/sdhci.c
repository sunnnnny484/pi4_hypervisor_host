...
#ifdef SDHCI_DEBUG
	if( !cnt ) {
		sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, 1, 1, "%s: reg %x, mask %x, val %x, rval %x", __func__, reg, mask, val, sdhci_in32( base + reg ) );
	}
#endif
	...
	if( hc->flags & SF_ICE_PERSIST ) { 
		if( rst & SDHCI_SYSCTL_SRA ) { 
			...
			if( sdhci_waitmask( hc, SDHCI_SYSCTL, SDHCI_SYSCTL_ICS, SDHCI_SYSCTL_ICS, 20000 ) != EOK ) { 
				sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, 1, 1, "%s:  wait ics timeout......", __func__ );
			} 
		} 
	} 
