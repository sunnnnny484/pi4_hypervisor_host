/*
 * $QNXLicenseC:
 * Copyright 2020, 2023-2025, BlackBerry Limited.
 *
...
options(const int argc, char *argv[])
{
    ...
    while (optind < argc) {
        ...
        while ((optind < argc)  &&  (*(optarg = argv[optind]) != '-')) {
            ...
            if (errno != EOK) {
                (void)MINIUART_SLOG_ERROR("Invalid serial port base address, errno: %d", errno);
                break;
            }
            devinit.port = val;
            if (*optarg == ',') {
                ...
                if (errno != EOK) {
                    (void)MINIUART_SLOG_ERROR("Invalid serial port irq, errno: %d", errno);
                    break;
                }
                devinit.intr = (unsigned)val;
            }
