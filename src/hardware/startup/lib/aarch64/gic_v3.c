...
static void gic_v3_gicc_init(const unsigned cpu)
{
	...
	else
	{
		...
		_Uint64t const aff3 = (cpu_aff >> 24) & 0xfful;
		...
		if (aff0 > 0x0ful) {
			_Uint64t const ctlr = aa64_sr_rd64(S3_0_C12_C12_4); // ARM_GICC_CTLR_EL1
			_Uint32t const gicd_typer = in32(gicd_paddr_base + ARM_GICD_TYPER);

			/* Idealy we would check that the RSS bit of ARM_GICD_TYPER and ICC_CTLR_EL1
			 * is 1, but the HW might not be able to indicate support when it is actually provided
			 */
			icc_sgi1r |= (aff0 & 0xf0ul) << (44-4);

			if ((gicd_typer & ARM_GICD_TYPER_RSS) == 0U) {
				kprintf("WARNING: cpu%d: aff0: %d and GICD_TYPER.RSS = 0\n", cpu, aff0);
			}
			if ((ctlr & ARM_GICC_CTLR_EL1_RSS) == 0U) {
				kprintf("WARNING: cpu%d: aff0: %d and ICC_CTLR_EL1.RSS = 0\n", cpu, aff0);
			}
		}
	...
	if (debug_flag > 1) {
		kprintf("cpu%d: GICv3 cpu interface 0x%l\n", cpu, gic_map->gic_cpu[cpu]);
	}
}
